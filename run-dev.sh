...
sudo podman run --rm --name pipe-front-dev -p 3000:3000 -u ${UID} -ti -v "$(pwd):/go/src/github.com/88250/pipe/" pipe-front-dev bash -c "cd console && npm run dev"
sudo podman run --rm --name pipe-go-dev -u ${UID} -ti -v "$(pwd):/go/src/github.com/88250/pipe/" pipe-go-dev bash
